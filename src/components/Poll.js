...
export class Poll extends Component {
  ...
  //Called when user votes to update pollvotes
  ...
  //Loads the poll when the page is loaded
  componentDidMount() {
    this.loadPoll();
  }
  ...
  //Calls api to update user vote on the poll
  userVote = (event) => {
    const user = JSON.parse(localStorage.getItem(Base64.encode("user")));
  ...
  //Displays toast message when user has already voted
  notifyHasVoted = () => {
    toast.error(
      <span>
        <i
  ...
  //Displays toast message when user votes for the first time
  notifyVoting = () => {
    toast.success(
      <span>
        <i
  ...
  //Calculates progress bar on poll answers
